...
- form 태그의 method 속성에 지정된 전송 방식으로 action 속성에 지정한 서버ㅓ로 입력한 값들을 전송한다.

9. **reset** : form에서 입력한 값을 모두 초기화 한다.

## 추가된 Type

1. 날짜 관련 Type

- **datetime** or **datetime-local** : 년/월/일/시/분/에 대한 데이터를 입력받을 수 있다.

%  ///?//////////////
## input 기본 속성

1. placeholer : 텍스트입력에 도움이 되는 힌드를 표시
2. autofocus: 요소에 마우스커서를 표시
3. required : 필수로 입력해야 되는 태그에 사용
4. reeadonly : 읽기 전용으로 설정
5. disabled : 비활성화

## 표준 & 접근성 향상
1. firldset: 양식의 소그룹
2. lengend : fieldset의 제목
3. label : 필드와 텍스트의 그룹
